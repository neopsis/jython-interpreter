...
public class JythonExecutor implements Runnable {
    ...
    protected synchronized JeditConsole getInterpreter() {
        ...
        if (interpreter == null) {
            ...
            sys.ps1 = new PyString(">>> ");
            sys.ps2 = new PyString("... ");
            ...
            if (useInternalLib) {
                ...
                String path = new File(new File(jar.getPath()).getParent(), "jython.jar").toString();

                sys.packageManager.addJar(path, true);
                sys.path.insert(0, new PyString(path + "/Lib"));
                sys.path.insert(0, new PyString(path));
            }
    ...
    protected void internalExecMacro(View view, String name) {
        ...
        PySystemState sys         = Py.getSystemState();
        PyObject      py_view     = ((view != null)
                                     ? Py.java2py(view)
                                     : Py.None);
        PyObject      py_buffer   = ((buffer != null)
                                     ? Py.java2py(buffer)
                                     : Py.None);
        PyObject      py_editPane = ((editPane != null)
                                     ? Py.java2py(editPane)
                                     : Py.None);
        PyObject      py_textArea = ((textArea != null)
                                     ? Py.java2py(textArea)
                                     : Py.None);
        ...
        if (ePlugin != null) {
            ...
            try {
                ...
                try {
                    ...
                    // cannot find the module in the sys.modules __dict__
                    // most likely because of compile errors. We expect the internal modules to be error-free
                    // for others the error should be reported in the jiconsole
                    return null;
                }
