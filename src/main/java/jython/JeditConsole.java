...
public class JeditConsole extends InteractiveInterpreter {
    ...
    public void doIO(String command, Console console, boolean addPrompt) {
        ...
        if (out.size() > 1) {
            ...
            console.printResult(resultText.substring(0, resultText.length() - ((command.length() == 0)
                    ? 0
                    : 1)));
        }
        ...
        if (err.size() > 1) {
            ...
            console.printErrorMsg(errorText.substring(0, errorText.length() - ((command.length() == 0)
                    ? 0
                    : 1)), FILENAME, 0);
        }
